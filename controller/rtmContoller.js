...
const RTMS_FILE = 'rtms';

// GET /api/v1/rtms - Get all RTMS
export const getRTMs = (req, res) => {
    try {
        const rtms = dataHelper.getData(RTMS_FILE);
        console.log('triggered rtm');
        res.json(rtms);
    } catch (error) {
...
const downloadRTM = async (req, res) => {
    try {
        ...
        const rtms = dataHelper.getData(RTMS_FILE);
        if (!id) {
            return res.status(400).json({ message: 'RTM ID is required' });
        }

        const rtm = rtms.find(p => p.id === id);

        if (!rtm) {
            return res.status(404).json({ message: 'RTM not found' });
        }

        // Get the file path
        const filePath = path.join(process.cwd(), rtm.filePath.slice(1));
...
// Check if file exists
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${ rtm.name }"`);
        ...
        console.error('Error in downloadRTM:', error);
        res.status(500).json({
...
const generateRTM = async (req, res) => {
    try {
        ...
        if (!projectId || !clientName) {
            return res.status(400).json({ error: 'projectId and clientName are required' });
        }
        ...
        const messages = [
            {
                role: 'system',
                content: 'You are a helpful assistant that generates a Requirements Traceability Matrix (RTM) from provided knowledge base content. ' +
                    'Use the information from the knowledge base to produce a comprehensive RTM for the given project and client. ' +
                    'Focus only on requirements relevant to the specified client. ' +
                    'Respond ONLY with the RTM data in JSON format (an array of objects), where each object represents one requirement mapping with all relevant fields (such as IDs, descriptions, assumptions, comments, etc.). ' +
                    'Do not include explanations or any text outside the JSON structure.',
            },
            {
                role: 'user',
        ...
        if (rtmData.length > 0) {
            const firstRowKeys = Object.keys(rtmData[0]).filter(key => key.toLowerCase() !== 'source');
            worksheet.columns = firstRowKeys.map(key => ({
                header: key,
                key: key,
                width: 30
        ...
        const fileName = `${clientName}_RTM_${projectId}.xlsx`;
        const filePath = path.join(outputDir, fileName);

        // Write the Excel file to disk
        await workbook.xlsx.writeFile(filePath);

        // **4. Update RTM records using dataHelper (FIXED)**
        // Load existing RTM records using dataHelper
        const allRtmRecords = dataHelper.getData(RTMS_FILE);

        // Determine a new unique RTM ID
        let newIdNumber = 1;
        if (allRtmRecords.length > 0) {
            for (const record of allRtmRecords) {
                if (record.id && record.id.startsWith('rtm-')) {
        ...
        // Create the new RTM record object
        const newRtmEntry = {
            id: newRtmId,
            projectId: projectId,
            client: clientName,
            ...
            filePath: `/files/rtm/${fileName}`
        };

        // Append the new record and save using dataHelper
        allRtmRecords.push(newRtmEntry);
        dataHelper.setData(RTMS_FILE, allRtmRecords);
